// win : CL /MD /Feapp /Iinclude e08.cpp /link /LIBPATH:lib\win /NODEFAULTLIB:MSVCRTD
...
// lin : c++ -std=c++0x -o app e08.cpp -I ./include -L./lib/lin -Wl,-rpath,./lib/lin/ -lglfw -lGL
// l32 : c++ -std=c++0x -o app e08.cpp -I ./include -L./lib/lin32 -Wl,-rpath,./lib/lin32/ -lglfw -lGL
...
const GLchar* fragmentSource =
	...
	"	outColor = vec4(material_ambient, 0.1);"
	"	outColor += vec4(material_diffuse, 0.1) * texture(textureSampler, Coord) * max(dot(light_direction, Normal), 0.0);"
	"	outColor += vec4(material_specular, 1.0) * pow(max(dot(R, view_direction), 0.0), shininess);"
	"float dist = gl_FragCoord.z / gl_FragCoord.w;"
	"float fogFactor = (25 - dist)/(25-0.01);"
...
void draw(GLFWwindow* window)
{
	...
	glm::mat4 model(1.f);
	model *= glm::translate(glm::mat4(1.f), glm::vec3(1.f, 0.f, -3.f));
	...
	//glDisable(GL_DEPTH_TEST); check(__LINE__);
