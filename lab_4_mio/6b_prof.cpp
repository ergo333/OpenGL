// win : CL /MD /Feapp /Iinclude e06.cpp /link /LIBPATH:lib\win /NODEFAULTLIB:MSVCRTD
// osx : c++ -std=c++11 -o app e06.cpp -I ./include -L ./lib/mac -lglfw3 -framework Cocoa -framework OpenGL -framework IOKit -framework CoreVideo
// lin : c++ -std=c++0x -o app e06.cpp -I ./include -L./lib/lin -Wl,-rpath,./lib/lin/ -lglfw -lGL
// l32 : c++ -std=c++0x -o app e06.cpp -I ./include -L./lib/lin32 -Wl,-rpath,./lib/lin32/ -lglfw -lGL
...
const GLchar* vertexSource =
	...
	"	vec3 R2 = reflect(-light_direction2, Normal);"
	"	Coord = vec2(coord.x, 1.0-coord.y);"
	"	Color = light_intensity*material_ambient;"
	"	Color += light_intensity2*vec3(.1,.1,.1);"
	"	Color += light_intensity2*material_diffuse * max(dot(light_direction2, Normal), 0.0);"
	...
	"	Color += light_intensity2*material_specular * pow(max(dot(R2, view_direction), 0.0), shininess);"
	"}";
...
const GLchar* fragmentSource =
	...
	"void main() {"
	"	outColor = texture(textureSampler, Coord) * vec4(Color, 1.0);"
	"}";
...
// creiamo la struttura per i materiali
...
void initialize_vao_from_file(size_t i, const std::string& inputfile)
{
	...
	std::string err = tinyobj::LoadObj(shapes, materials, inputfile.c_str(), "assets/sphere/"); 
	// carica il modello definito da inputfile, l'ultimo parametro e' il path in cui cercare il material
	...
	// (non dev'essere mecessariamente così)
...
{
	glGenTextures(2, texture); // in realtà ne useremo una sola
...
void draw(GLFWwindow* window)
{
	...
	glm::vec3 light_direction2 = glm::normalize(glm::vec3(0.f, 5.f, 3.f));
	glm::vec3 light_intensity2;
	if(light2)
	 light_intensity2 = glm::vec3(1.f, 1.f, 0.f);
	else
 	light_intensity2 = glm::vec3(0.f, 0.f, 0.f);
	...
	glUniform3fv(glGetUniformLocation(shaderProgram, "light_direction2"), 1, &light_direction2[0]);
	glUniform3fv(glGetUniformLocation(shaderProgram, "light_intensity2"), 1, &light_intensity2[0]);
