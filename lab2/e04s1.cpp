...
void draw(GLFWwindow* window)
{
	...
	glUniform1i(glGetUniformLocation(shaderProgram, "textureSampler"), 0);
        glm::mat4 projection = glm::ortho(-2.0f,2.0f,-2.0f,2.0f,1.0f,10.0f);

	glm::mat4 view = glm::lookAt(glm::vec3(2.5f, 2.5f, 2.5f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));


	glm::mat4 model = glm::rotate(glm::mat4(1.f), 2*PI, glm::vec3(0.0f, 1.0f, 0.0f));
	...
	projection = glm::perspective(PI/4, 1.f/1.f, 1.0f, 10.0f);
	glUniformMatrix4fv(glGetUniformLocation(shaderProgram, "projection"), 1, GL_FALSE, &projection[0][0]);
	glDrawElements(GL_TRIANGLES, 6*2*3, GL_UNSIGNED_INT, 0); // facce * triangoli per faccia * vertici per triangolo
...
#endif
	window = glfwCreateWindow(800, 400, "cg-lab", NULL, NULL);
	if(!window)
	{
		glfwTerminate();
		exit(EXIT_FAILURE);
