...
const GLchar* fragmentSource =
	...
	"	outColor = vec4(material_ambient, 0.3);"
	"	outColor += vec4(material_diffuse, 0.3) * texture(textureSampler, Coord) * max(dot(light_direction, Normal), 0.0);"
	"}";
...
const GLchar* fragmentSource2 =
	...
	"uniform vec3 view_position;"	
	"void main() {"
	"	outColor = vec4(material_diffuse, 1);"
	"}";
...
*/
	...
	//Traslazione terra 
	model *= glm::translate(glm::mat4(1.f), glm::vec3(3.f*sin(t), 0.f, 3.f*cos(t)));
