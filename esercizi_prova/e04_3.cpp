...
GLuint trans_flag;
glm::vec3 camera_position(2.5f, 2.5f, 2.0f);
...
static void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
		...
		if (key == GLFW_KEY_T && action == GLFW_PRESS) trans_flag=(trans_flag+1)%2;
}
...
void draw(GLFWwindow* window)
{
	...
	if(trans_flag) {
		camera_position = camera_position + glm::vec3(0.0f, dt, 0.0f);
		camera_target = camera_target + glm::vec3(0.0f, dt, 0.0f);
	}
